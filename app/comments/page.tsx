...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[12px] lg:text-[13px] font-bold ${s.cls}`}>
      {s.label}
    </span>
  );
}
        ...
        <div className="flex gap-3 px-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className="text-[13px] lg:text-[14px] font-bold tracking-wider text-aring-ink-400 flex-shrink-0">No.{no}</span>
              <span className="text-[13px] lg:text-[14px] font-bold tracking-wider text-aring-ink-500 truncate">{brand}</span>
            ...
            {/* 메타: 작성자 · 시간 · 조회수 · 좋아요 */}
            <div className="mt-1 flex items-center gap-2 text-[12px] lg:text-[13px] font-normal text-aring-ink-400 whitespace-nowrap overflow-hidden">
            ...
            {/* 댓글 본문 — 메인 콘텐츠라 한 단계 더 큼 */}
            <p
              className="mt-1.5 text-[14px] lg:text-[15px] font-normal text-aring-ink-700 leading-[1.5] overflow-hidden"
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 2,
  ...
  return (
    <Link href={`/items/${c.product_id}`} className="block">
      <div className="flex items-center gap-3 px-6 py-3 hover:bg-aring-ink-50 transition-colors border-b border-aring-ink-100 last:border-b-0">
        {/* No */}
        <div className="w-10 text-center text-[15px] lg:text-[15px] leading-[1.5] text-aring-ink-400 font-bold flex-shrink-0">
          {no}
        </div>
        ...
        {/* 작성자 */}
        <div className="w-20 flex-shrink-0 text-[12px] lg:text-[13px] text-aring-ink-600 font-semibold truncate">
          {c.user_name || '익명'}
        </div>
      ...
      <div className="relative w-full max-w-[440px] min-h-screen overflow-hidden sm:my-6 sm:min-h-[900px] sm:rounded-[36px] sm:shadow-phone lg:max-w-[1200px] lg:my-0 lg:min-h-screen lg:rounded-none lg:shadow-none lg:overflow-visible">
        <div className="pb-28 lg:pb-10">
          ...
          <div className="px-5 lg:px-8 pt-3 pb-2">
            ...
            {!loading && (
              <p className="mt-1 text-[15px] lg:text-[15px] font-bold text-aring-ink-700">전체 댓글 {total}개</p>
            )}
          </div>
          ...
          {!loading && comments.length > 0 && (
            <div className="hidden lg:block mt-2 mx-6 rounded-2xl bg-white shadow-card border border-aring-ink-100 overflow-hidden">
              {/* 헤더 */}
              <div className="flex items-center gap-3 px-6 py-2.5 bg-aring-ink-50 border-b border-aring-ink-100 text-[15px] lg:text-[15px] font-bold text-aring-ink-500 uppercase tracking-wide">
