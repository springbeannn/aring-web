...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      {err && (
        <div className="bg-rose-50 border border-rose-200 text-rose-700 text-[13px] px-4 py-3 rounded-lg">
          {err}
        </div>
      )}
      ...
      <section className="bg-white rounded-2xl shadow-card border border-aring-ink-100 p-6">
        ...
        {loading ? (
          ...
          <p className="text-[13px] text-aring-ink-500">데이터 없음</p>
        ) : (
            ...
            {recentListings.map((it) => (
              <li key={it.id} className="flex items-center gap-3 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-bold text-aring-ink-400 truncate">{it.brand ?? '브랜드 미상'}</p>
                  <p className="text-[14px] font-bold text-aring-ink-900 truncate">{it.detail ?? it.shape ?? '한 짝'}</p>
                </div>
                <span className="text-[12px] text-aring-ink-500 shrink-0">{fmtDate(it.created_at)}</span>
                <span className="text-[12px] font-semibold text-aring-ink-700 shrink-0">{statusLabel(it.status)}</span>
                <Link
                  href={`/items/${it.id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[12px] font-semibold text-aring-green hover:underline shrink-0"
                >
                  상세 →
                </Link>
              </li>
            ...
            {recentUsers.map((u) => (
              <li key={u.user_id} className="flex items-center justify-between py-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[12px] text-aring-ink-500 truncate">{u.email}</p>
                </div>
                <span className="text-[12px] text-aring-ink-500 shrink-0">{fmtDate(u.created_at)}</span>
              </li>
            ))}
          </ul>
        )}
...
function KpiCard({ label, value, loading }: { label: string; value?: number; loading: boolean }) {
  return (
    <div className="bg-white rounded-2xl shadow-card border border-aring-ink-100 p-6">
      <p className="text-[13px] font-bold text-aring-ink-500">{label}</p>
      {loading ? (
