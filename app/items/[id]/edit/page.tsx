  ...
  if (notOwner) return (
    <main className="min-h-screen flex justify-center bg-white">
      <div className="w-full max-w-[440px] min-h-screen px-5 pt-20 text-center">
        <p className="text-[16px] font-bold text-aring-ink-900">수정 권한이 없어요</p>
        <Link href="/" className="mt-4 inline-flex px-5 py-2.5 rounded-pill bg-aring-ink-900 text-white text-[15px] lg:text-[15px] font-bold">홈으로</Link>
      </div>
    </main>
  );
      ...
      <div className="relative w-full max-w-[440px] bg-white overflow-hidden min-h-screen sm:my-6 sm:rounded-[36px] sm:shadow-phone lg:max-w-[600px]">
        ...
        <div className="px-5 pt-2 pb-32">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-[20px] font-bold text-aring-ink-900">상품 수정</h1>
          </div>
          ...
          <div className="space-y-4">
            {/* 브랜드 */}
            <div>
              <label className="block text-[15px] lg:text-[15px] font-bold text-aring-ink-400 tracking-wider uppercase mb-1.5">브랜드</label>
              <input
            ...
            {/* 가격 */}
            <div>
              <label className="block text-[15px] lg:text-[15px] font-bold text-aring-ink-400 tracking-wider uppercase mb-1.5">희망 가격</label>
              <input
            ...
            {/* 등록자 한마디 */}
            <div>
              <label className="block text-[15px] lg:text-[15px] font-bold text-aring-ink-400 tracking-wider uppercase mb-1.5">등록자 한마디</label>
              <textarea
                value={story} onChange={e => setStory(e.target.value)}
                placeholder="이 귀걸이에 대해 한마디 남겨주세요"
                rows={4} maxLength={1000}
                className="w-full px-4 py-3 rounded-2xl border border-aring-ink-200 text-[16px] lg:text-[16px] lg:font-normal text-aring-ink-900 placeholder:text-aring-ink-400 outline-none focus:border-aring-ink-500 transition resize-none"
              />
              <p className="mt-1 text-right text-[15px] lg:text-[15px] leading-[1.5] text-aring-ink-400">{story.length}/1000</p>
            </div>

            {/* 지역 */}
            <div>
              <label className="block text-[15px] lg:text-[15px] font-bold text-aring-ink-400 tracking-wider uppercase mb-1.5">지역</label>
              <input
          ...
          <button
            ...
            className={`mt-8 w-full py-4 rounded-2xl font-bold text-[16px] transition active:scale-95 ${saving ? 'bg-aring-ink-100 text-aring-ink-400' : 'bg-aring-ink-900 text-white shadow-cta'}`}
          >
            {saving ? '저장 중...' : '저장하기'}
          </button>
