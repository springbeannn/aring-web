...
function PopularCard({ row, rank, bgClass }: { row: Listing; rank: number; bgClass?: string }) {
  return (
    <Link
      href={`/items/${row.id}`}
    ...
    >
      <div
        className={`relative aspect-square overflow-hidden ${bgClass ?? getPastelClassById(row.id)}`}
      >
        {row.photo_url && (
          <img
            src={row.photo_url}
      ...
      ">
        <div className="pb-28 lg:pb-10">
          ...
          {loading ? (
            ...
            <div className="px-5 lg:px-8 py-16 text-center">
              <p className="text-[15px] lg:text-[15px] font-bold text-aring-ink-900">아직 등록된 한 짝이 없어요</p>
              <p className="mt-1 text-[15px] lg:text-[15px] leading-[1.5] text-aring-ink-500">첫 등록자가 되어 보세요</p>
              <Link
                href="/register"
                className="mt-4 inline-flex items-center justify-center px-5 py-2.5 rounded-pill bg-aring-ink-900 text-white text-[15px] lg:text-[15px] font-bold"
              >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 px-5 lg:px-8">
              {(() => {
                ...
                return rows.map((row, idx) => (
                  <PopularCard key={row.id} row={row} rank={idx + 1} bgClass={bgs[idx]} />
                ));
              })()}
            </div>
          )}
