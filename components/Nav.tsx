  ...
  const item = (key: Tab, label: string, href: string, icon: React.ReactNode, dot?: 'unread' | 'new') => {
    ...
    return (
      <Link
        href={href}
        aria-label={label}
        className={`flex-1 text-[12px] lg:text-[13px] font-semibold ${isActive ? 'text-aring-ink-900' : 'text-aring-ink-500'} active:opacity-70`}
      >
        <div className="flex flex-col items-center gap-1 py-2 relative">
          {dot && (
            <span
              ...
              style={{ background: 'linear-gradient(135deg, rgba(251,200,220,.55), rgba(197,221,240,.55))', filter: 'blur(8px)', zIndex: -1 }}
            />
          )}
          {icon}
          <span>{label}</span>
