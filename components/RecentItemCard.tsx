    ...
    return (
          <Link
                ...
                <div className="px-3 py-3">
                        <p className="text-[10.5px] font-bold tracking-wider text-aring-ink-500 truncate">{it.brand}</p>p>
                        <p className="mt-0.5 text-[13px] font-bold text-aring-ink-900 truncate">{it.name}</p>p>
                  {it.story && <p className="mt-1 text-[10.5px] text-aring-ink-500 truncate">· {it.story}</p>p>}
                        <div className="mt-2 flex items-center justify-between">
                                  <span className="text-[12px] font-bold text-aring-ink-900">{formatKRW(it.price)}</span>span>
                                  <div className="flex items-center gap-2">
                                    {typeof it.viewCount === 'number' && it.viewCount > 0 && (
                                <span className="inline-flex items-center gap-0.5 text-[10.5px] text-aring-ink-400">
                                                <IconEye />
                                  {it.viewCount}
                                </span>span>
                                              )}
                                              <button
                                                              onClick={handleLike}
                                                              aria-label="찜하기"
                                                              className={['inline-flex items-center gap-1 text-[10.5px] transition', liked ? 'text-aring-accent' : 'text-aring-ink-500'].join(' ')}
                                                            >
